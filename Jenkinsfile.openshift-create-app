node {
    ...
    withEnv(["PATH+OC=${tool 'oc4.6'}"]){
        ...
        stage('oc create'){
            sh "oc process -f https://raw.githubusercontent.com/moricom2/eGovFrame-sht/main/eap73-openjdk11-basic-s2i.yaml -l app=${APPLICATION_NAME},application=${APPLICATION_NAME} APPLICATION_NAME=${APPLICATION_NAME} SOURCE_REPOSITORY_URL=${SOURCE_REPOSITORY_URL} SOURCE_REPOSITORY_REF=${SOURCE_REPOSITORY_REF} CONTEXT_DIR=${CONTEXT_DIR} -o yaml | oc create -f -"                
        }
